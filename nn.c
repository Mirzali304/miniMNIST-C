...
void read_mnist_images(const char *filename, unsigned char **images, int *nImages) {
    ...
    *nImages = __builtin_bswap32(*nImages);
    ...
    // 验证魔数（图片文件应为 2051）
    if (magic != 2051) {
        fprintf(stderr, "Error: Invalid magic number in image file (expected 2051, got %d)\n", magic);
        fclose(file);
        exit(1);
    }

    // 验证图片尺寸（MNIST 图片固定 IMAGE_SIZE x IMAGE_SIZE）
    if (rows != IMAGE_SIZE || cols != IMAGE_SIZE) {
        fprintf(stderr, "Error: Image size mismatch: expected %dx%d, got %dx%d\n",
                IMAGE_SIZE, IMAGE_SIZE, rows, cols);
        fclose(file);
        exit(1);
    }

    // 分配内存：每张图片占 IMAGE_SIZE * IMAGE_SIZE 字节
    *images = malloc((*nImages) * IMAGE_SIZE * IMAGE_SIZE);
    if (!*images) {
        fprintf(stderr, "Error: Failed to allocate memory for %d images\n", *nImages);
        fclose(file);
        exit(1);
    }

    size_t bytes_to_read = (*nImages) * IMAGE_SIZE * IMAGE_SIZE;
    size_t bytes_read = fread(*images, 1, bytes_to_read, file);
    if (bytes_read != bytes_to_read) {
        fprintf(stderr, "Error: Failed to read image data (expected %zu bytes, got %zu)\n",
...
void read_mnist_labels(const char *filename, unsigned char **labels, int *nLabels) {
    ...
    *nLabels = __builtin_bswap32(*nLabels);
    ...
    // 验证魔数（标签文件应为 2049）
    if (magic != 2049) {
        fprintf(stderr, "Error: Invalid magic number in label file (expected 2049, got %d)\n", magic);
        fclose(file);
        exit(1);
    }
    ...
    *labels = malloc(*nLabels);
    if (!*labels) {
        fprintf(stderr, "Error: Failed to allocate memory for %d labels\n", *nLabels);
        fclose(file);
        exit(1);
    }

    size_t bytes_read = fread(*labels, 1, *nLabels, file);
    if (bytes_read != (size_t)(*nLabels)) {
        fprintf(stderr, "Error: Failed to read label data (expected %d bytes, got %zu)\n",
                *nLabels, bytes_read);
        free(*labels);
        fclose(file);
        exit(1);
